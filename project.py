def get_reply(message):
    ...
    if re.search(r"\b(hi|hello|hey|hii)\b", msg):
        return "Hi there! What would you like to know about the college? LIKE : FEE.   ,COURSE ,ADMISSION ,FACILITY ,LOCATION ,FACILITY, CONTACT,"

    if "course" in msg or "courses" in msg:
        return (
            "Here are the main courses available:\n"
            "- B.Tech (CSE, CSE Specialization s, ECE, ME, CE)\n"
            "- BBA, MBA\n"
            "- M.Tech\n"
        )

    if "admission" in msg:
        return ("Admissions are done through the VITEEE.   entrance exam followed by counselling.\n"
            "You need to fill out the online form on the official website. Seat allotment is based on your VITEE rank.")

    if "fee" in msg or "fees" in msg:
        return (
            "Here’s the approx fee structure:\n"
            "- B.Tech: around ₹2,00,000 CAT1 per year\n
            "- B.Tech: around ₹3,00,000 CAT1 per year\n"
            "- B.Tech: around ₹4,00,000 CAT1 per year\n"
            "- B.Tech: around ₹4,50,000 CAT1 per year\n"
            "- B.Tech: around ₹5,00,000 CAT1 per year\n"
    ...
    if "location" in msg or "where" in msg:
        return (
            'The college is situated near Bhopal, in Sehore district, Aastha village — pincode 466114.'
        )
